...
repositories_with_description = [repository for repository in repositories if (repository['description'] is not None)]

all_topics = [val for sublist in repositories_with_description if len(sublist['topics']) for val in sublist['topics']]
most_common_topic = Counter(all_topics).most_common(10)
most_common_topic_list = [ topic[0] for topic in most_common_topic]
oxford_comma_separated_topics = ", ".join(most_common_topic_list[:-2] + [" e ".join(most_common_topic_list[-2:])])
